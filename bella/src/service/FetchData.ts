...
class FetchData {
  ...
  async getAccessory(categoryId: string, code: string) {
    return await axios.get<Array<IAccessoryType>>(
      `/product/?category=${categoryId}&code=${code}`
    );
  }
  ...
  async getAccessoryType(categoryId: string) {
    return await axios.get<Array<IAccessoryTypesType>>(
